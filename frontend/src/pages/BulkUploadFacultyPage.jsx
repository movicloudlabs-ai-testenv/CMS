...
export default function BulkUploadFacultyPage() {
    ...
    try {
      ...
      for (let i = 1; i < lines.length; i++) {
        ...
        const normalizedObj = {
          ...
          designation: obj.designation || obj.role || obj.Role || obj.Designation || 'Assistant Professor',
          highestQualification: obj.highestQualification || obj.qualification || obj.Qualification || obj['Highest Qualification'] || '',
          specialization: obj.specialization || obj.Specialization || '',
          university: obj.university || obj.University || '',
          yearsOfExperience: obj.yearsOfExperience || obj.years_of_experience || obj['Years of Experience'] || '0',
          employmentType: obj.employmentType || obj.employment_type || obj['Employment Type'] || 'Full-Time',
        };
      ...
      if (items.length === 0) {
        setError('No valid faculty records found in CSV. Make sure you have at least a "name" column.');
      } else {
        setParsedData(items);
      }
    } catch (err) {
  ...
  const handleUpload = async () => {
    ...
    setUploading(true);
    try {
      const res = await fetch(buildApiUrl('/faculty/bulk-import'), {
        method: 'POST',
      ...
      alert(`Successfully imported ${result.count} faculty application requests!\nThey will appear in the Admissions pending queue for approval.`);
      navigate('/faculty');
    } catch (err) {
      console.error(err);
  ...
  return (
    <Layout title="Bulk Upload Faculty">
      <div className="space-y-4">
        ...
        <div className="bg-white rounded-lg shadow p-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-lg font-bold text-gray-900">Bulk Faculty Creation</h1>
              <p className="text-xs text-gray-600 mt-0.5">Upload a CSV file containing multiple faculty records to request admissions approval.</p>
            </div>
          </div>
          <button
              ...
              >
                {uploading ? (
                  ...
                  <>
                    ...
                    <span>Submit {parsedData.length > 0 ? `${parsedData.length} Faculty` : 'CSV'}</span>
                  </>
                )}
              </button>
            </div>
            ...
            <div className="bg-white rounded-lg shadow p-6 h-full flex flex-col">
              ...
              {parsedData.length > 0 ? (
                    ...
                    <tbody className="divide-y divide-gray-100 text-gray-700">
                      {parsedData.slice(0, 10).map((row, i) => (
                        <tr key={i} className="hover:bg-gray-50/50">
                          ...
                          <td className="py-2.5 px-3">{row.designation}</td>
                          <td className="py-2.5 px-3">{row.yearsOfExperience} years</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                ...
                <div className="flex-1 flex flex-col items-center justify-center text-center p-8 text-gray-400 border border-dashed border-gray-200 rounded-xl min-h-[300px]">
                  ...
                  <p className="text-xs text-gray-500 max-w-sm mt-1">Upload a CSV file to inspect the faculty records and headers before importing them.</p>
                  ...
                  <div className="mt-6 text-left bg-slate-50 border border-slate-100 rounded-lg p-4 max-w-md">
                    ...
                    <ul className="text-[10px] text-slate-600 space-y-1 list-disc list-inside">
                      <li><code className="font-bold text-indigo-600">name</code> / <code className="font-bold text-indigo-600">fullName</code> - Full name of faculty (Required)</li>
                      ...
                      <li><code className="font-bold text-indigo-600">designation</code> - Lecturer, Assistant Professor, etc.</li>
                      <li><code className="font-bold text-indigo-600">yearsOfExperience</code> - Years of experience (number)</li>
                    </ul>
                  </div>
                </div>
              )}
